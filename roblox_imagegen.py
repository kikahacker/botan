...
import os
import logging
import asyncio, io, json, math, hashlib, datetime, traceback
from typing import Any, Dict, List, Optional, Tuple

import httpx
from PIL import Image, ImageDraw, ImageFont

# --- Static ready item directory (forced to item_images) ---
READY_ITEM_DIR = os.path.join(os.getcwd(), 'item_images')
os.makedirs(READY_ITEM_DIR, exist_ok=True)

# ---- logger ----
DEBUG_IMAGEGEN = str(os.getenv('DEBUG_IMAGEGEN', 'false')).strip().lower() in ('1','true','yes','y','on')
...
logger.info(f"[imagegen] cwd={os.getcwd()} READY_ITEM_DIR={os.path.abspath(READY_ITEM_DIR)} DEBUG={DEBUG_IMAGEGEN}")
...
def _log_exception(prefix: str, e: Exception):
    # Always log stack to help debug production crashes
    logger.error(f"{prefix}: {e.__class__.__name__}: {e}\n{traceback.format_exc()}")
...
WRITE_READY_ITEM_IMAGES = str(os.getenv('WRITE_READY_ITEM_IMAGES', 'false')).strip().lower() in ('1','true','yes','y','on')
ROBUX_PREFIX = os.getenv('ROBUX_PREFIX', 'R$')
KEEP_INPUT_ORDER = str(os.getenv('KEEP_INPUT_ORDER', 'false')).strip().lower() in ('1','true','yes','y','on')
...
GAP_IMAGE_PRICE = int(os.getenv('GAP_IMAGE_PRICE', os.getenv('GAP_PRICE_LINE', str(GAP_PRICE_LINE))))
...
# Optional override: force thumb size regardless of tile (useful for experiments)
THUMB_SIZE_OVERRIDE = os.getenv('THUMB_SIZE_OVERRIDE')  # e.g. "420x420"
...
def _load_price_rules():
    ...
    try:
        ...
        _log_debug(f'PRICE_RULES parse fail: {e}')
        return None
...
# Prefer LANCZOS when downscaling for sharpness
    ...
    for ext in ('.png', '.jpg', '.jpeg', '.webp'):
        ...
        if os.path.exists(p):
            try:
                ...
                _log_debug(f"[local] hit {aid}{ext} size={im.size}")
                return im
            except Exception as e:
                _log_exception(f"[local] open fail for {p}", e)
    _log_debug(f"[local] miss {aid}")
    return None
...
def _write_ready_item(aid: int, im: Image.Image):
    ...
    try:
        ...
        os.replace(tmp, p)
        _log_debug(f"[local] saved {p}")
    except Exception as e:
        _log_exception("[local] save fail", e)

# ---- network image fetch with cache ----
...
async def _download_image_with_cache(url: str) -> Optional[Image.Image]:
    ...
    if b:
        try:
            ...
            _log_debug(f"[thumb] cache HIT key={key[:10]} url={url[:80]}")
            return im
        except Exception as e:
            _log_exception("[thumb] cache decode fail", e)
    _log_debug(f"[thumb] cache MISS url={url}")
    ...
    for attempt in range(4):
        ...
        try:
            ...
            if status != 200:
                _log_info(f"[thumb] GET {url[:80]} status={status} attempt={attempt+1}")
            r.raise_for_status()
            data = r.content
            await cache.set_bytes(key, data)
            _log_debug(f"[thumb] downloaded {len(data)}b from {url[:80]}")
            return Image.open(io.BytesIO(data)).convert('RGBA')
        except Exception as e:
            _log_exception(f"[thumb] fetch fail try {attempt + 1}", e)
            await asyncio.sleep(0.2 * (attempt + 1))
    return None

# ---- roblox thumbnails batching ----
...
async def _fetch_thumbs(ids: List[int], size: str='150x150') -> Dict[int, Image.Image]:
    ...
    _log_info(f"[thumb] input={len(ids)} local_hits={len(result)} need_fetch={len(left)} size={size}")
    if left:
        _log_debug(f"[thumb] missing sample={left[:10]}")
    if not left:
        return result
    ...
    async def one_batch(ch: List[int]):
        ...
        try:
            r = await client.get(
                base,
                params={'assetIds': ','.join(map(str, ch)), 'size': req_size, 'format': 'Png', 'isCircular': 'false', 'returnPolicy': 'PlaceHolder'},
                headers=_auth_headers(),
                timeout=httpx.Timeout(HTTP_TIMEOUT, connect=HTTP_CONNECT_TIMEOUT, read=HTTP_READ_TIMEOUT)
            )
            _log_info(f"[thumb] batch {ch[0]}..{ch[-1]} count={len(ch)} status={r.status_code} size={req_size}")
            r.raise_for_status()
            def parse(js):
                urls, pending = ({}, [])
            ...
            _log_debug(f"[thumb] got urls={len(urls)} pending={len(pending)}")
            for d in THUMB_REPOLL_DELAYS:
                ...
                rr = await client.get(
                    base,
                    params={'assetIds': ','.join(map(str, pending)), 'size': req_size, 'format': 'Png', 'isCircular': 'false', 'returnPolicy': 'PlaceHolder'},
                    headers=_auth_headers(),
                    timeout=httpx.Timeout(HTTP_TIMEOUT, connect=HTTP_CONNECT_TIMEOUT, read=HTTP_READ_TIMEOUT)
                )
                _log_debug(f"[thumb] repoll status={rr.status_code} pend_before={len(pending)} delay={d}")
            ...
            _log_exception(f"[thumb] batch fail {ch[:3]}..", e)
            return {}
    ...
    async def dl(aid, url):
        async with sem_dl:
            ...
            if im is not None:
                ...
                _log_info(f"[thumb] empty for {aid} (url={url[:80]})")

    await asyncio.gather(*(dl(a, u) for a, u in url_map.items()))
    _log_info(f"[thumb] fetch done total={len(ids)} have={len(result)} missing={len(ids)-len(result)}")
    return result
...
def _render_tile(it: Dict[str, Any], thumb: Image.Image, tile: int) -> Image.Image:
    ...
    d.text((left + (pill_w - w_text) // 2,
            top + (pill_h - price_font.getbbox('Ag')[3]) // 2), price_text, fill=(0, 0, 0, 255), font=price_font)
...
# ---- grid rendering (tile default 420) ----

DEFAULT_TILE = int(os.getenv('DEFAULT_TILE', '420'))

async def _render_grid(items: List[Dict[str, Any]], tile: int = DEFAULT_TILE, title: str='Items', username: Optional[str]=None, user_id: Optional[int]=None) -> bytes:
    n = len(items)
    ...
    # allow overriding thumb size via env
    size = THUMB_SIZE_OVERRIDE or f'{tile}x{tile}'
    _log_info(f"[grid] start items={n} tile={tile} size={size}")
    ...
    _log_info(f"[grid] done items={n} tile={tile} bytes={out.tell()}")
    return out.getvalue()

# ---- public API (defaults now 420) ----

async def generate_full_inventory_grid(items: List[Dict[str, Any]], tile: int = DEFAULT_TILE, pad: int = 0, username: Optional[str]=None, user_id: Optional[int]=None, title: Optional[int]=None) -> bytes:
...
async def generate_inventory_preview(tg_id: int, roblox_id: int, categories_limit: int=8, username: Optional[str]=None) -> bytes:
    ...
    for arr in (data.get('byCategory') or {}).values():
        items.extend(arr)
    return await _render_grid(items, tile=DEFAULT_TILE, title='Инвентарь', username=username, user_id=tg_id)
